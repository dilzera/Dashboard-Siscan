...
- **Main Sections:**
    ...
    - **Patient Data:** Comprehensive listing of all records with specific filters (name, CPF, Cartão SUS, sex, BI-RADS) and pagination.
...
- **Sidebar Navigation:** Collapsible left sidebar (dark gradient, 260px expanded / 60px collapsed / 0px hidden) replaces horizontal tabs. Three-state toggle: expanded → collapsed → hidden (with floating restore button). Navigation items: Visão Geral, Auditoria de Risco, Auditoria de Outliers, Indicadores, Navegação da Paciente, Dados do Paciente, Unidade e Prestador, Interoperabilidade. "Configuração" section with collapsible submenu containing "Gerenciar Acessos" (visible only for secretaria/distrito). Active tab tracked via `dcc.Store(id='active-sidebar-tab')`. CSS in `assets/sidebar.css`. Sidebar toggle via hamburger button; floating `sidebar-toggle-floating` button appears when sidebar is fully hidden. All tab content rendered as hidden divs, toggled via `display` style.
- **Global Filter Mutual Exclusion:** BI-RADS and Priority global filters are mutually exclusive. Selecting one clears and disables the other. When global BI-RADS is set, the local BI-RADS filter in Patient Data tab is also disabled.
- **Table Horizontal Scroll:** All data tables wrapped with `.table-scroll-wrapper` CSS class providing consistent horizontal scroll, custom scrollbar styling, and max-height of 600px.
