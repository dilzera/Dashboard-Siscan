...
- **Sidebar Navigation:** Collapsible left sidebar (dark gradient, 260px expanded / 60px collapsed) replaces horizontal tabs. Navigation items: Visão Geral, Auditoria de Risco, Auditoria de Outliers, Indicadores, Navegação da Paciente, Dados do Paciente, Unidade e Prestador, Interoperabilidade. "Configuração" section with collapsible submenu containing "Gerenciar Acessos" (visible only for secretaria/distrito). Active tab tracked via `dcc.Store(id='active-sidebar-tab')`. CSS in `assets/sidebar.css`. Sidebar toggle via hamburger button. All tab content rendered as hidden divs, toggled via `display` style.
